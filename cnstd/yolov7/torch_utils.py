...
def git_describe(path=Path(__file__).parent):  # path must be a directory
    ...
    cmd = ['git', '-C', str(path), 'describe', '--tags', '--long', '--always']
    try:
        return subprocess.check_output(cmd, stderr=subprocess.STDOUT).decode()[:-1]
...
def select_device(device='', batch_size=None):
    ...
    s = f'YOLOv7 🚀 {date_modified()} torch {torch.__version__} '  # string
