...
def detect_pse(seg_maps, threshold=0.5, threshold_k=0.55, boxes_thres=0.01):
    ...
    bboxes, scores = mask_to_boxes_pse(result_map, seg_maps[0, :, :], min_score=boxes_thres)
...
def main(image_dir, ckpt_path, output_dir, ctx=None, num_kernels=3):
    if not ctx:
        ctx = mx.cpu()
    else:
        ctx_id = eval(ctx)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
    main(image_dir, ckpt_path, output_dir, gpu_list)
